...
class VerifyViewModel @Inject constructor(
    ...
    fun verify(verifyRequest: VerifyRequest){
        authUseCase.performVerify(verifyRequest = verifyRequest).onEach { result ->
            when(result) {
                is Resource.Loading -> {
                    Log.i("VerifyViewModel", "I am here, Loading")
                }
                is Resource.Error -> {
                    error.postValue("${result.message}")
                    successful.postValue(false)
                    Log.i("VerifyViewModel", "I am here, Error ${result.message}")
                }
                is Resource.Success -> {
                    successful.postValue(true)
                    Log.i("VerifyViewModel", "I am here, Success ${result.data?.message}")
                }
            }
